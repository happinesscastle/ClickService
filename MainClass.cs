...
namespace ClickServerService
{
    internal class MainClass
    {
        ...
        public void Decript_Connection_String()
        {
            ...
            //{
            //  DBPATH = System.IO.File.ReadAllText(path1);
            //}
            //else
            //{
            //  string path2 = AppDomain.CurrentDomain.BaseDirectory + "\\db.txt";
            //  if (System.IO.File.Exists(path2))
            //    DBPATH = Decrypt(System.IO.File.ReadAllText(path2));
            //  else
        ...
        public string getShamsiForDataTextBox(DateTime dateT) => p.GetYear(dateT).ToString() + "/" + int.Parse(p.GetMonth(dateT).ToString()) + "/" + p.GetDayOfMonth(dateT).ToString();

        public string getShamsiAndTime(DateTime dateT) => p.GetYear(dateT).ToString() + "/" + int.Parse(p.GetMonth(dateT).ToString()) + "/" + p.GetDayOfMonth(dateT).ToString() + " " + dateT.Hour + ":" + dateT.Minute + ":" + dateT.Second;
        ...
        public void ErrorLogTemp(string P)
        {
            try
            {
                WriteToFileError("Date: " + DateTime.Now.ToString() + " - P: " + P);
            }
            catch { }
        }
        ...
        public string Start_Synchronize()
        {
            ...
            try
            {
                ...
                sqlConnection.ConnectionString = DBPath();
                sqlConnection.Open();
