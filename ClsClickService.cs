    ...
    public class ClsClickService //: ServiceBase
    {
        ...
        public void AppLoadMain()
        {
            try
            {
                ...
                else
                {
                    ...
                    DataTable byGameCenter = this.objMain.ServerConfig_GetByGameCenter(objMain.ID_GameCenter_Local_Get(), MultiRun_AP_ID);
                    ...
                    if (byGameCenter.Rows.Count > 0)
                    {
                        ...
                        this.WriteToFile("txtServerIp:" + txtServerIp + $",chbAP{MultiRun_AP_ID}:" + chbAP1.ToString() + $",txtAp{MultiRun_AP_ID}_IP:" + txtAp1_IP + ",cblRepeatConfig:" + (object)cblRepeatConfig + "cblValidateReceivedData:" + (object)cblValidateReceivedData + ",chbDecreasePriceInLevel2:" + chbDecreasePriceInLevel2.ToString() + ",chbShowAllRecive:" + chbShowAllRecive.ToString() + ",chbShowAllSend:" + chbShowAllSend.ToString());
                    }
                    else
                    ...
                    if (chbAP1)
                    {
                        ...
                        {
                            IpAp1 = txtAp1_IP;
                            TCp_IP_Thread_1 = new Thread(new ThreadStart(this.Recive_StartTcpIp));
        ...
        private void OnElapsedTime(object source, ElapsedEventArgs e)
        {
            if (chbAP1)
            {
                try
                {
                    if ((this.Send_Main(IpAp1, "check", clientAp1) == 1) && TCp_IP_Thread_1.IsAlive)
                    {
                        this.objMain.ServerConfig_SetAp1Status(this.objMain.ID_GameCenter_Local_Get(), true, MultiRun_AP_ID);
                    }
                    else
                    {
                        this.WriteToFile(DateTime.Now.ToString() + $": Ap{MultiRun_AP_ID} is Disconnect.");
                        ...
                        {
                            this.WriteToFile($"Ap{MultiRun_AP_ID} NoStart :" + (object)ex);
                        }
                        try
                        {
                            clientAp1.Close();
                        ...
                        {
                            this.WriteToFile(DateTime.Now.ToString() + $":Ap{MultiRun_AP_ID} Close -" + ex.Message);
                        }
                        IpAp1 = txtAp1_IP;
                ...
                {
                    this.WriteToFile(DateTime.Now.ToString() + $": Ap{MultiRun_AP_ID} :" + ex.Message);
                }
            }
            //if (chbAP2)
            //{
        ...
        public void Recive_StartTcpIp()
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        this.WriteToFile($"Co_StartTcpIp_{MultiRun_AP_ID}");
                        clientAp1.Close();
                    }

                    clientAp1.Connect(IpAp1, port);
                    Console.WriteLine($"clientAp{MultiRun_AP_ID}.Connect (IP : {IpAp1} , Port: {port})");
                    ...
                    try
                    {
                        ...
                        while ((count = stream.Read(numArray, 0, numArray.Length)) > 0)
                        {
                            ...
                            try
                            {
                                ...
                                Console.WriteLine("+Recive :<" + IpAp1 + " : <- " + str2);
                                Console.ForegroundColor = ConsoleColor.White;
                                ...
                                try
                                {
                                    ...
                                    for (int index = 0; index < strArray.Length; ++index)
                                    {
                                        ...
                                        if (DispStringSplit.Length > 0)
                                        {
                                            ...
                                            if (this.Process_ReciveData(DispStringSplit, MultiRun_AP_ID) == "true")
                                                this.Recive_DisplayText(DispStringSplit, $"P{MultiRun_AP_ID}");
                ...
                if (!(str1 == ""))
                {
                    ...
                    else
                    {
                        for (int index2 = 0; index2 < tcpRepeatCount; ++index2)
                        {
                            ...
                            this.Send_DisplayText(str2, $"P{MultiRun_AP_ID}", t_SwiperName, t_CardmacAddress);
                    ...
                    for (int index2 = 0; index2 < byState.Rows.Count; ++index2)
                    {
                        ...
                        {
                            this.Send_DisplayText(str2, $"P{MultiRun_AP_ID}", "", "");
                        ...
                        if (DispStringRecive.ToUpper().Contains("[KA=") || DispStringRecive.ToLower().Contains("selftest"))
                        {
                            ...
                            for (int index2 = 0; index2 < stateForChangePrice.Rows.Count; ++index2)
                            {
                                ...
                                if (addressByChargeRate.Rows.Count > 0)
                                {
                                    ...
                                    {
                                        this.Send_DisplayText(str4, $"P{MultiRun_AP_ID}", "", "");
                                    ...
                                    for (int index3 = 0; index3 < TCP_RepeatCount + 5; ++index3)
                                    {
                                        this.Send_DisplayText(str5, $"P{MultiRun_AP_ID}", "", "");
