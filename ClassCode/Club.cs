...
namespace ClickServerService.ClassCode
{
    internal class Club
    {
        ...
        public int Club_Point_Process_UpdateCardPoint(string Card_GUID, int Point, int ID_Club_Member_Type)
        {
            try
            {
                SqlConnection sqlConnection = new SqlConnection(clsMain.DBPath());
                sqlConnection.Open();
        ...
        public void Club_Point_Process(string Card_GUID, int ID_Club_Point_Type, int Price, int Point_Old, int ID_Game)
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    SqlCommand selectCommand = new SqlCommand("Club_Point_Process", connection) { CommandType = CommandType.StoredProcedure };
                ...
                if ((uint)Point > 0U)
                {
                    ...
                    Club_Point_History_Insert(clsMain.ID_GameCenter_Local_Get(), ID_Club_Campaign, ID_Club_Member_Type, Card_GUID, Point, Point_Old, Description);
        ...
        public bool Club_CheckIn_Process(string Card_GUID)
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    SqlCommand selectCommand = new SqlCommand("Club_CheckIn_Process", connection) { CommandType = CommandType.StoredProcedure };
                    selectCommand.Parameters.AddWithValue("@Card_GUID", Card_GUID);
                    new SqlDataAdapter(selectCommand).Fill(dataTable);
                }
                if (dataTable.Rows.Count > 0)
